...
FROM (SELECT b.business_id AS business_id, b.name, b.city, b.state, scaleReview(r.rating, cast(r.review_time AS string)) AS scale_score
  FROM BusinessStatic AS b, ReviewStatic AS r
  WHERE b.business_id == r.business_id
    AND r.review_time IS NOT NULL
  ) AS scale_table
WHERE scale_table.scale_score != -1 and r.reviews_time BETWEEN ${hiveconf:start_time} AND ${hiveconf:end_time}
GROUP BY scale_table.business_id
Order by b.state ASC, r.scaleScore DESC;
